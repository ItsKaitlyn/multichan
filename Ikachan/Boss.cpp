...
void HitTwoCharBoss(EVENT_SCR *event_scr, CARET_SPAWNER *caret_spawner)
{
	char flag = 0;
	char touch = FALSE; //???
	...
	//Check if Ikachan is touching Iron Head
	if (tMC.x < (gBoss.x + gBoss.hit.right) &&
		tMC.x > (gBoss.x + gBoss.hit.right - 0x1400) &&
		tMC.y < (gBoss.y + gBoss.hit.bottom - 0x1000) &&
		(tMC.y + 0x4000) > (gBoss.y + gBoss.hit.top + 0x1000))
	{
		tMC.x = gBoss.x + gBoss.hit.right;
		tMC.xm = 0;
		flag |= 1;
		touch = TRUE;
	}
	if (tMC.y < (gBoss.y + gBoss.hit.bottom) &&
		tMC.y > (gBoss.y + gBoss.hit.bottom - 0x1400) &&
		tMC.x < (gBoss.x + gBoss.hit.right - 0x1000) &&
		(tMC.x + 0x4000) > (gBoss.x + gBoss.hit.left + 0x1000))
	{
		if (tMC.ym < -100)
			PlaySoundObject(SOUND_ID_HITHEAD, SOUND_MODE_PLAY);
		tMC.y = gBoss.y + gBoss.hit.bottom;
		tMC.ym = 0;
		flag |= 2;
		touch = TRUE;
	}
	if ((tMC.x + 0x4000) > (gBoss.x + gBoss.hit.left) &&
		(tMC.x + 0x4000) < (gBoss.x + gBoss.hit.left + 0x1400) &&
		tMC.y < (gBoss.y + gBoss.hit.bottom - 0x1000) &&
		(tMC.y + 0x4000) > (gBoss.y + gBoss.hit.top + 0x1000))
	{
		tMC.x = gBoss.x + gBoss.hit.left - 0x4000;
		tMC.xm = 0;
		flag |= 4;
		touch = TRUE;
	}
	if ((tMC.y + 0x4000) > (gBoss.y + gBoss.hit.top) &&
		(tMC.y + 0x4000) < (gBoss.y + gBoss.hit.top + 0x1400) &&
		tMC.x < (gBoss.x + gBoss.hit.right - 0x1000) &&
		(tMC.x + 0x4000) > (gBoss.x + gBoss.hit.left + 0x1000))
	{
		tMC.airborne = FALSE;
		tMC.y = gBoss.y + gBoss.hit.top - 0x4000;
		if (tMC.ym > 0)
			tMC.ym = 0;
		flag |= 8;
		touch = TRUE;

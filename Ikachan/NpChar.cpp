...
void HitTwoCharNpChar(NPCHAR *npc, EVENT_SCR *event_scr, CARET_SPAWNER *caret_spawner)
{
	for (int i = 0; i < MAX_NPCS; i++, npc++)
	{
		BOOL touch = FALSE;
		...
		//Check for non-solid contact
		if (npc->type != 3 && (npc->type != 2 || tMC.shock != 0))
		{
			if (npc->type == 0 &&
				tMC.no_event == 0 &&
				tMC.x < npc->x + 0x1000 &&
		...
		{
			//Solid contact
			if (tMC.x < (npc->x + 0x3400) && tMC.x > (npc->x + 0x2000) && tMC.y < (npc->y + 0x3000) && tMC.y > (npc->y - 0x3000))
			{
				tMC.x = npc->x + 0x3400;
				tMC.xm = 0;
				tMC.flag |= 1;
			...
			if (tMC.y < (npc->y + 0x3400) && tMC.y > (npc->y + 0x2000) && tMC.x < (npc->x + 0x3000) && tMC.x > (npc->x - 0x3000))
			{
				if (tMC.ym < -100)
					PlaySoundObject(SOUND_ID_HITHEAD, SOUND_MODE_PLAY);
				tMC.y = npc->y + 0x3400;
			...
			if (tMC.x > (npc->x - 0x3400) && (tMC.x + 0x3FF) < (npc->x - 0x2000) && tMC.y < (npc->y + 0x3000) && tMC.y > (npc->y - 0x3000))
			{
				tMC.x = npc->x - 0x3400;
				tMC.xm = 0;
				tMC.flag |= 4;
			...
			switch (npc->type)
			{
				case 2:
					...
					if (npc->act_no == 1)
					{
						...
						if (tMC.x > npc->x)
							tMC.xm = 0x400;
						DamageTwoChar(caret_spawner, npc_damage[npc->code_char]);
					}
					
					//Check if we should hurt the NPC
					if (tMC.flag != 0 && tMC.unit == 1 && tMC.flag != 8 || (tMC.flag & 2) && (tMC.equip & 1))
					{
						if (npc_defense[npc->code_char] > tMC.level)
						{
							//The NPC was too strong
							if ( !tMC.no_event )
								PlaySoundObject(SOUND_ID_NODMG, SOUND_MODE_PLAY);
